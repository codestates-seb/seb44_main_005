        ...
        <div className='fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-black bg-opacity-40 z-50'>
            <div className='flex flex-col w-[600px] h-[500px] space-y-[30px] rounded-lg bg-white drop-shadow-lg'>
                ...
                <div className='flex flex-row justify-center space-x-[50px]'>
                    <p>닉네임</p>
                    <input className='border-[2px] border-[#CCCCCC] rounded' type='text' defaultValue={updatedNickname} onChange={(e) => setUpdatedNickname(e.target.value)} />
                </div>
                <div className='flex flex-row justify-center space-x-[50px]'>
                    <p>연락처</p>
                    <input className='border-[2px] border-[#CCCCCC] rounded' type='text' defaultValue={updatedPhoneNumber} onChange={(e) => setUpdatedPhoneNumber(e.target.value)} />
                </div>
                <div className='flex justify-center pt-[70px]'>
                    <button className='bg-[#4771B7] text-white p-3 rounded-lg' type='button' onClick={handleEditComplete}>편집 완료</button>
                </div>
            </div>
        </div>
    );
